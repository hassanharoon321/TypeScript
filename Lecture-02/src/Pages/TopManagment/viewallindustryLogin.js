    ...
    <div className=" mt-5">
      {/* Heading Starts */}
                ...
                <div className="row pt-3">
                    ...
                    <h1 className="top-managment-jobs-head">Jobs For Top Managment</h1>
        <h3 className="showing-99-jobs">Showing {props.topmanagementReducer.topmanagement.jobs&&props.topmanagementReducer.topmanagement.jobs.length>0?props.topmanagementReducer.topmanagement.jobs.length:""} Top Managment Jobs</h3>
                    </div>
      ...
      <div className="row mt-3">
        ...
        <div className="col-md-9 over">
          ...
          {
             props.topmanagementReducer.topmanagement.jobs&&props.topmanagementReducer.topmanagement.jobs.length>0?props.topmanagementReducer.topmanagement.jobs.filter((val)=>{
                if(searchItem==""){
                    return val
                  }else if(val.job_title.toLowerCase().includes(searchItem.toLowerCase())){
                    return val
                  }
             }).map(job=>(
              <Link to={`/jobdetaillog?id=${job.id}`} className="top-manag-link-def">
              <div className="shadow mb-5" style={{borderRadius:"10px"}}>
                <div className="managment-box-head">
                  <h2 className="hiring-color">
                    Hiring {job.job_title!==null&&job.job_title!==undefined?job.job_title:""}
                  </h2>
                  <p className="hiring-down-color">{job.rec!==null&&job.rec!==undefined?job.rec:""}</p>
                </div>
                <div className="experience-div">
                  <div className="experience-div-01">
                    <img src={Briefcase} />
                    <span className="left-to-icon-top-manag">
                    {job.exp!==null&&job.exp!==undefined?job.exp:""}
                    </span>
                  </div>
                  <div className="experience-div-02">
                    <img src={Location} />
                    <span className="left-to-icon-top-manag">
                    {job.city!==null&&job.city!==undefined?job.city:""} , {job.country!==null&&job.country!==undefined?job.country:""}
                    </span>
                  </div>
                  <div className="experience-div-03">
                    <img src={Pen} />
                    <span className="left-to-icon-top-manag">
                    {job.func!==null&&job.func!==undefined?job.func:""}
                    </span>
                  </div>
                </div>
                <div className="box-last-main">
                  <p>
                  {job.description!==null&&job.description!==undefined?job.description:""}
                  </p>
