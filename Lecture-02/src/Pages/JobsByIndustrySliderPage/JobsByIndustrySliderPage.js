...
function JobsByIndustrySliderPage(props) {
    useEffect(() => {
        loadGetHomeJob()
                        ...
                        <div className="row mb-5">
                            {props.dashboardReducer.dashboard.jobs_by_industry && props.dashboardReducer.dashboard.jobs_by_industry.length > 0 ? props.dashboardReducer.dashboard.jobs_by_industry.map((job) => (
                                <div className="col-lg-4 col-md-6 mb-5 py-auto ">
                                    <Link to={`/topmanagement?id=${job.id}?industry=${job.name}?count=${job.job_count}`} className="link-tag-home">
...
export default connect(mapStateToProps, mapDispatchToProps)(JobsByIndustrySliderPage)
